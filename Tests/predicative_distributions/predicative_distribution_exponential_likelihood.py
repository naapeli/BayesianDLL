...
with Model() as joint_posterior_model:
    rate = RandomParameter("rate", HalfCauchy(prior_scale), torch.tensor([1], dtype=torch.float64), sampler="auto", gamma=10)
    likelihood = ObservedParameter("likelihood", Exponential(rate), data)
    prior_predicative = sample_prior_predicative(n_samples, n_warmup)
    posterior_predicative = sample_posterior_predicative(n_samples, n_warmup)

with Model() as joint_posterior_model:
    rate = RandomParameter("rate", HalfCauchy(prior_scale), torch.tensor([1], dtype=torch.float64), sampler="auto")
    prior_samples = sample(n_samples, n_warmup)

with Model() as joint_posterior_model:
    rate = RandomParameter("rate", HalfCauchy(prior_scale), torch.tensor([1], dtype=torch.float64), sampler="auto", gamma=5)
    likelihood = ObservedParameter("likelihood", Exponential(rate), data)
    posterior_samples = sample(n_samples, n_warmup)
...
rate_samples = prior_samples["rate"]
...
plt.subplot(1, 2, 2)
rate_samples = posterior_samples["rate"]
