...
samples = samples.squeeze(-1).flatten(0, 1).int()
...
plt.bar(prior.distribution.state_space.values, prior.distribution.pdf(prior.distribution.state_space.values.float()).squeeze(), alpha=0.5, label="prior")
plt.tight_layout()
plt.legend()
plt.show()
