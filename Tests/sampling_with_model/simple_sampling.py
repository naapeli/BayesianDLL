...
from BayesianDLL import Model
from BayesianDLL.Samplers import sample
from BayesianDLL import RandomParameter
...
samples = sample(10000, 1000, normal_model)
...
plt.bar(normal_model.params["z"].distribution.state_space, frequencies, alpha=0.5, label="Binomial")
x = torch.linspace(-5, 5, 1000)
plt.plot(x, Uniform(-4, -2).pdf(x))
plt.plot(x, Normal(0, 1).pdf(x))
plt.scatter(normal_model.params["z"].distribution.state_space, Binomial(5, 0.3).pdf(normal_model.params["z"].distribution.state_space.values))
plt.legend()

plt.subplot(1, 2, 2)
plt.plot(samples["x"], label="Uniform", alpha=0.5)
plt.plot(samples["y"], label="Normal", alpha=0.5)
plt.plot(samples["z"], label="Binomial", alpha=0.5)
plt.legend()
plt.show()
