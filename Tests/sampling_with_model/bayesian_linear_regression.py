...
with Model() as linear_model:
    # Priors
    prior_intercept = RandomParameter("intercept", Normal(0, 20), torch.tensor([10], dtype=torch.float64), sampler="auto")
    prior_slope = RandomParameter("slope", Normal(0, 20), torch.tensor([10], dtype=torch.float64), sampler="auto")
    prior_sigma = RandomParameter("sigma", HalfCauchy(10), torch.tensor([10], dtype=torch.float64), sampler="auto")
...
plt.tight_layout()
plt.savefig("Tests/sampling_with_model/linear_trace_and_posteriors.png")
...
plt.legend()
plt.tight_layout()
plt.savefig("Tests/sampling_with_model/linear_fit.png")
