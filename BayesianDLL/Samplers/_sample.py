...
def sample(n_samples, warmup_length, model=None, progress_bar=True):
    ...
    _progress_bar = tqdm(range(1, n_samples + warmup_length + 1)) if progress_bar else range(1, n_samples + warmup_length + 1)
...
def sample_posterior_predicative(n_samples, warmup_length, model=None, progress_bar=True, warmup_per_sample=20, init_point_variance=10):
    model = _active_model._active_model if model is None else model
    trace = sample(n_samples, warmup_length, model, progress_bar)
    return sample_predicative(trace, n_samples, model, progress_bar, warmup_per_sample, init_point_variance)

def sample_prior_predicative(n_samples, warmup_length, model=None, progress_bar=True, warmup_per_sample=20, init_point_variance=10):
    model = _active_model._active_model if model is None else model
    old_observed = model.observed_params
    model.observed_params = {}
    trace = sample(n_samples, warmup_length, model, progress_bar)
    model.observed_params = old_observed
    return sample_predicative(trace, n_samples, model, progress_bar, warmup_per_sample, init_point_variance)

def sample_predicative(trace, n_samples=None, model=None, progress_bar=True, warmup_per_sample=20, init_point_variance=10):
    model = _active_model._active_model if model is None else model
    ...
    for i in _progress_bar:
        ...
        for name, sampler in samplers.items():
            ...
            theta = init_point_variance * torch.randn_like(init_value, dtype=init_value.dtype)
            no_squeeze = theta.ndim == 1
            theta = theta if no_squeeze else theta.unsqueeze(0)
            for m in range(warmup_per_sample + 1):
                theta = sampler.step(theta, m < warmup_per_sample)
    ...
    predicative_samples = {name: torch.stack(samples) for name, samples in predicative_samples.items()}
