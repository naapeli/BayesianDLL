...
from BayesianDLL.Distributions import Normal, Uniform, Exponential
from BayesianDLL.Evaluation import Graphics
from BayesianDLL.GP import LatentGP, RBF, Periodic, gp_predictive
...
with Model() as model:
    ...
    z = RandomParameter("z", Normal(0.0, 1.0), shape=x.numel())  # the latent variables of the gaussian process (created automatically in the GP if not defined in the model)
    ...
    f = LatentGP(
        "f",
        inputs,
        # RBF(lengthscale=lengthscale, variance=variance),
        Periodic(lengthscale=lengthscale, variance=variance, period=period),
        latent=z,
    )
    with plate("observations", x):
        ObservedParameter("y", Normal(f, noise_variance), y)
...
# blocks = [
#     SamplingBlock(model, ["z"], sampler_type="nuts", sampler_params={"max_depth": 4}),
#     SamplingBlock(model, ["log_lengthscale", "log_variance", "log_period"], sampler_type="nuts", sampler_params={"max_depth": 4})
# ]
blocks = None
trace = model.sample(200, 200, blocks=blocks)  # , max_depth=6, delta=0.8
print(trace.summary())

new_inputs = torch.linspace(-0.1, 1.1, 50)
predictions = gp_predictive(f, trace, new_inputs, n_samples=20)

Graphics.plot_posterior(trace, method="kde", aggregate=True)

function_samples = trace["f"].reshape(-1, x.numel())
...
ax.fill_between(x, function_quantiles[0], function_quantiles[2], color="tab:blue", alpha=0.2, label="Latent 90% interval")
ax.plot(x, function_quantiles[1], color="tab:blue", label="Latent posterior median")
ax.fill_between(new_inputs, predictive_quantiles[0], predictive_quantiles[2], color="tab:orange", alpha=0.2, label="Predictive 90% interval")
...
ax.set(title="Latent GP posterior and predictive distribution", xlabel="x", ylabel="y")
ax.legend()
plt.show()
