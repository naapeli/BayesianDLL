...
ess_drift_tail = effective_sample_size(sample_dict_drift, method="tail")["theta"].item()
print(f"  BayesianDLL (mean ESS) : {ess_drift_mean:.1f}")
print(f"  BayesianDLL (bulk ESS) : {ess_drift_bulk:.1f}")
print(f"  BayesianDLL (tail ESS) : {ess_drift_tail:.1f}")
...
az_drift_tail = float(az.ess(az_data_drift, method="tail")["x"].values)
print(f"  ArviZ       (mean ESS) : {az_drift_mean:.1f}")
print(f"  ArviZ       (bulk ESS) : {az_drift_bulk:.1f}")
print(f"  ArviZ       (tail ESS) : {az_drift_tail:.1f}")
